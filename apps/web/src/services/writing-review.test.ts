...
describe('offersCard', () => {
  const finding = (kind: FindingKind, card?: { gap?: boolean }): WritingFinding => ({
    kind,
    note: 'n',
    ...(card ? { card: { study: 's', back: { English: 'e', Korean: 'k' }, ...card } } : {}),
  });

  it('offers nothing when the finding carries no card', () => {
    for (const kind of FINDING_KINDS) expect(offersCard(finding(kind))).toBe(false);
  });

  /**
   * The measurement this rule exists for: a grammar finding's card front is
   * often a description — "accord du participe passé avec être" — which
   * pattern offers used to hide and nothing would now.
   */
  it('withholds a grammar finding\'s ordinary card', () => {
    expect(offersCard(finding('grammar', {}))).toBe(false);
    expect(offersCard(finding('grammar', { gap: false }))).toBe(false);
  });

  /** The one grammar case that is real evidence rather than a judgement. */
  it('offers a grammar finding\'s gap card', () => {
    expect(offersCard(finding('grammar', { gap: true }))).toBe(true);
  });

  /** Every other kind is about a word or a phrasing in the first place. */
  it('offers every non-grammar card, gap or not', () => {

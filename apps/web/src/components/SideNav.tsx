...
export default function SideNav({ collapsed, onToggle }: Props) {
  ...
  return (
    ...
    >
      {/* Logo, with the collapse toggle underneath. Every row below leads with
          a column exactly as wide as the logo (3.6rem = 57.6px at size 30) with
      ...
      <div className="px-2 pt-5 pb-2 space-y-1">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 pl-2 pr-3 py-2 rounded-lg hover:bg-[var(--color-muted)]/20 transition-colors"
      ...
      <nav className="flex-1 space-y-1 px-2">
        {navItems.map((item) => (
          <a
            ...
            className="flex items-center gap-3 pl-2 pr-3 py-2.5 rounded-lg font-mono text-sm transition-colors hover:bg-[var(--color-muted)]/20"
            style={
              item.active
      ...
      <div className="pb-4 space-y-1 px-2">
        ...
        <div className="relative space-y-1" ref={popoverRef}>
          ...
          <button
            ...
            className="w-full flex items-center gap-3 pl-2 pr-3 py-2.5 rounded-lg font-mono text-sm transition-colors hover:bg-[var(--color-muted)]/20"
            style={{ color: 'var(--color-text)' }}
          >
            {user ? (
              <>
                <span className="w-[3.6rem] flex justify-center flex-shrink-0">
                  {user.photoURL ? (
                    <img src={user.photoURL} alt="User avatar" className="w-7 h-7 rounded-full" referrerPolicy="no-referrer" />
                  ) : (
                    <span

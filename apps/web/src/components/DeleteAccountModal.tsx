 ...
 * authoritative, but leaving a deleted user's study language and streak on the
 * device makes a completed deletion look unfinished.
 *
 * The theme is deliberately kept: it is a preference for this browser, not
 * data about the person, and resetting it would look like a bug.
 */
...
export default function DeleteAccountModal({ onClose }: { onClose: () => void }) {
  ...
  // DELETE reads as an instruction in one language and noise in the other —
  // and it confirms *which* account is going.
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // The account is already gone server-side, so this only clears the
      // client's session; a failure here must not read as a failed deletion.
  ...
  return (
    ...
    >
      ...
      >
        ...
        {error && <p className="text-sm" style={{ color: 'var(--color-error, #c0392b)' }}>{error}</p>}

...
interface Options {
   ...
   *
   * ⚠️ **This is what goes to the model, and it is not the interface
   * language.** The definition and examples generated here are *written onto
   * the card* and never regenerated, so they have to match the language the
   * rest of that card is in. Someone running Amgi in English while studying
   * Japanese from Korean backs must get Korean depth, or a single card ends up
   * half in each language, permanently.
   */
...
export function useCardEnrichment({
  ...
  const enrich = useCallback(async (kind: EnrichKind) => {
    ...
    try {
      ...
      // which is what keeps depth on `fine` about penalties. The deck's
      // language picks which stored side that sense is read from.
      ...
      const res = await fetch(`/api/explain/${route}`, {
        ...
        // `nativeLanguage` is the route's own parameter name and it means the
        // language to write in — the deck's, never the interface's.

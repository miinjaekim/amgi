...
```
Amgi는 새로 배운 단어를 물어보고, 이해하고, 잊기 직전에 다시 복습하는 것까지
한 곳에서 끝내는 언어 학습 앱입니다.

보통은 뜻이 궁금하면 AI에게 묻고, 외우려면 따로 암기 앱에 옮겨 적습니다.
Amgi는 그 두 단계를 하나로 합쳤습니다. 단어를 입력하면 번역에서 끝나지 않고
뉘앙스, 격식, 쓰임새, 한자 풀이, 실제 예문까지 설명해 주고, 그 설명을 그대로
...
베타 기간 동안 모든 기능은 무료이며, 광고나 트래킹은 없습니다.
```

### English

```
Amgi is a language-learning app that takes you from asking about a new word,
to understanding it, to reviewing it again right before you'd forget it —
all in one place.

Normally you ask an AI when you want to know a meaning, then copy it into a
separate flashcard app to memorize it. Amgi combines both steps. Type in a
...
Everything is free during the beta, with no ads or tracking.
```

...
- [x] **Speed up search/review (streaming)** — Depth and examples calls on the Learn screen now stream via `generateContentStream`. A typewriter animation (6 chars/frame, ~360 chars/sec) reveals content progressively while the model is mid-response, then flushes instantly when the stream ends so total load time is unchanged. Mobile continues using the non-streaming JSON endpoints. Caching/prefetching flashcards for the review queue is still outstanding.
